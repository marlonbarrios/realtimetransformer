...
export function useHandleSessionHistory() {
  ...
  function handleAgentToolEnd(details: any, _agent: any, _functionCall: any, result: any) {
    ...
    // If it's an image generation result, attach it to the last assistant message
    console.log('[handleAgentToolEnd] Function call:', _functionCall.name, 'Result:', parsedResult);
    if (_functionCall.name === 'generate_image' && parsedResult?.imageUrl) {
      console.log('[handleAgentToolEnd] ✅ Image generation result:', parsedResult);
      const lastAssistantMessage = extractLastAssistantMessage(details?.context?.history);
      console.log('[handleAgentToolEnd] Last assistant message:', lastAssistantMessage);
      if (lastAssistantMessage?.item_id) {
        console.log('[handleAgentToolEnd] ✅ Updating transcript item with image data for:', lastAssistantMessage.item_id);
        updateTranscriptItem(lastAssistantMessage.item_id, { 
          data: parsedResult 
        });
      } else {
        console.log('[handleAgentToolEnd] ❌ No assistant message found to attach image to');
      }
    } else {
      // For other function calls, add as breadcrumb
      addTranscriptBreadcrumb(
    ...
    // ALWAYS add as breadcrumb too (backup display method)
    if (_functionCall.name === 'generate_image' && parsedResult?.imageUrl) {
      console.log('[handleAgentToolEnd] 🔄 Also adding image as breadcrumb backup');
      addTranscriptBreadcrumb(
        `🎨 Imagen generada: ${_functionCall.name}`,
        parsedResult
      );
    }
  }
